    ...
    public class CustomFarmingReduxAPI : ICustomContentAPI
    {
        ...
        public Item getCustomObject(string id)
        {
            CustomMachine machine =
                new CustomMachine(CustomFarmingReduxMod.machines.Find(m => m.fullid == id || m.legacy == id));
        ...
        public List<Tuple<string, Texture2D, int, int>> getMachinesForItem(StardewValley.Object item)
        {
            ...
            foreach (CustomMachineBlueprint blueprint in CustomFarmingReduxMod.machines)
            {
                ...
                {
                    var material = recipe.materials.Find(i =>
                        i.index == -999 || i.index == item.ParentSheetIndex || item.Category == i.index);
                    if (material is IngredientBlueprint ing)
                        result.Add(new Tuple<string, Texture2D, int, int>(blueprint.name, blueprint.getTexture(),
                            blueprint.tileindex, ing.stack));
                }
            }
        ...
        {
            var result = CustomObjectData.collection.Find(c =>
                c.Value.sdvId == dummy.ParentSheetIndex && dummy.bigCraftable.Value == c.Value.bigCraftable);
            if (result is KeyValuePair<string, CustomObjectData> kvp && kvp.Value is CustomObjectData cod)
                return new Tuple<Item, Texture2D, Rectangle, Color>(cod.getObject(), cod.texture, cod.sourceRectangle,
                    cod.color);
        ...
        public void addContentPack(string folderName, string fileName, IModHelper helper = null,
            Dictionary<string, string> options = null)
        {
            ...
            if (pack is CustomFarmingPack)
                foreach (CustomMachineBlueprint blueprint in pack.machines)
                {
                    ...
                    {
                        toCrafting.AddOrReplace(blueprint.fullid,
                            $"{blueprint.crafting}/Home/130/true/null/{blueprint.fullid}");
                        CustomFarmingReduxMod.craftingrecipes.AddOrReplace(blueprint.fullid, 0);
                    }

                    if (blueprint.forsale && (blueprint.condition == null ||
                                              PyTK.PyUtils.CheckEventConditions(blueprint.condition)))
